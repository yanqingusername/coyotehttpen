...
class Category extends Adminbase
{
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            ...
            foreach ($result as $k => $v) {
                ...
                $v['catname'] = '<a data-full="true" data-open="' . url('edit', ['id' => $v['id']]) . '"">' . $v['catname'] . '</a>';
                if ($v['type'] == 1) {
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            switch ($data['type']) {
                //单页
                case 1:
                    $fields = ['parentid', 'catname', 'catdir', 'type', 'modelid', 'image', 'description', 'url', 'setting', 'listorder', 'letter', 'status', 'bottom_status', 'picname', 'subpicname'];
                    $scene = 'page';
                    break;
                //列表
                case 2:
                    $fields = ['parentid', 'catname', 'catdir', 'type', 'modelid', 'image', 'imagem', 'imageico', 'imagecat', 'description', 'url', 'setting', 'listorder', 'letter', 'status', 'bottom_status', 'picname', 'subpicname'];
                    $scene = 'list';
                    break;
                //链接
                case 3:
                    $fields = ['parentid', 'catname', 'catdir', 'type', 'image', 'description', 'url', 'setting', 'listorder', 'letter', 'status', 'bottom_status'];
                    $scene = 'link';
                    break;
                default:
                    $this->error('栏目类型错误~');
    ...
    public function edit()
    {
        if ($this->request->isPost()) {
            ...
            $status = $this->modelClass->editCategory($data, ['parentid', 'catname', 'catdir', 'type', 'modelid', 'image', 'imagem', 'imageico', 'imagecat', 'description', 'url', 'setting', 'listorder', 'letter', 'status', 'bottom_status', 'picname', 'subpicname']);
            if ($status) {
                if (isModuleInstall('member')) {
                    //更新会员组权限
                    model("cms/CategoryPriv")->update_priv($catid, $data['priv_groupid'], 0);
