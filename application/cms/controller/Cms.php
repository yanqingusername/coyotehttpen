...
class Cms extends Adminbase
{
    ...
    public function classlist()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $k => $v) {
                ...
                $v['inputtime'] = date('Y-m-d H:i:s', $v['inputtime']);
                if (isset($v['thumb'])) {
        ...
        $fielddata = ModelField::where('modelid', $modelid)->where('liststatus', 1)->order('listorder,id')->select();
        $this->assign('fielddata', $fielddata);
        $has_thumb = ModelField::where('modelid', $modelid)->where('status', 1)->where('name','thumb')->count();
        $this->assign('has_thumb', $has_thumb);
        return $this->fetch();
    }
        ...
        if ($this->request->isPost()) {
            ...
            if ($category['type'] == 2) {
                ...
                foreach ($fieldList as $v){
                    if (!empty($v['setting']['menusource']['menuid'])){
                        $menulistmodel = $category = getCategory($v['setting']['menusource']['menuid']);
                        $tablename = $this->Cms_Model->getModelTableName($menulistmodel['modelid']);
                        $v['setting']['menusource']['menulist'] = Db::name($tablename)->where('catid', $v['setting']['menusource']['menuid'])->select();
                    }
                    $menuList[] = $v;
                }
                if (!$this->Cms_Model->table_exists($tablename)) {
                ...
                if (empty($category['modelid'])) {
                    ...
                    //主表
                    $res= Db::name($tablename)->where('catid', $catid)->find();
                    if ($res) {
                        $fieldList = $this->Cms_Model->getFieldList($modelid, $res['id']);
                    }else{
                        $fieldList = $this->Cms_Model->getFieldList($modelid, 0);
                    }
                    $this->assign([
                        'catid' => $catid,
                        'fieldList' => $fieldList,
    ...
    public function getLink(){
        ...
        $tabledata =   Db::name('discrist')->where(['parentid'=>$pid])->select();
