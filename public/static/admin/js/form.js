layui.use(['layer', 'form', 'tableSelect', 'dragsort', 'tagsinput', 'colorpicker'], function() {
    ...
    $('.layui-color-box').each(function() {
        var that = $(this).closest(".layui-form-field-label").find(".test-form-input");
        colorpicker.render({
            elem: $(this),
            color: that.val(),
            done: function(color) {
    ...
    $('.js-upload-image,.js-upload-images').each(function() {
        ...
        uploader.on('uploadSuccess', function(file, response) {
            ...
            if (response.code == 0) {
                if ($multiple) {
                    if ($input_file.val()) {
                        $input_file.val($input_file.val() + ',' + response.id).trigger('change');
                    } else {
                        $input_file.val(response.id).trigger('change');
                    }
