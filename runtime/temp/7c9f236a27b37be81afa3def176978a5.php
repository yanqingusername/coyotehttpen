<?php /*a:1:{s:97:"/www/wwwroot/kyden.web.zhongwangyingtong.com/application/attachment/view/attachments/cropper.html";i:1596677558;}*/ ?>
<!doctype html>
<html lang="en">
...
<head>
    ...
    <link rel="stylesheet" href="/static/libs/cropper/cropper.min.css">
    <link rel="stylesheet" href="/static/libs/cropper/main.css">
...
<body style="padding: 10px;">
        ...
        <div class="layui-row layui-col-space10">
            <div class="layui-col-md8 layui-col-sm8">
                <div class="img-container">
                    <img id="image" src="<?php echo htmlentities(app('request')->get('url')); ?>" alt="Picture">
                </div>
            </div>
    ...
    <script src="/static/libs/cropper/cropper.min.js"></script>
    ...
    layui.use(['layer', 'jquery'], function() {
        ...
        function doUpload(filename, fileobj, index) {
            ...
            $.ajax({
                url: '<?php echo url("attachment/upload/upload", ["dir" => "images", "module" => "admin"]); ?>',
                type: 'POST',
                data: formData,
                cache: false,
                async: false,
                ...
                success: function(res) {
                    if (res.code == 0) {
                        ...
                        parent.$("." + arr[1] + "-" + arr[0]).attr('src', res.path).attr('data-original', res.path).removeClass("thumb-" + arr[0]).addClass(arr[1] + "-" + res.id);
                        var ids = parent.$('#' + arr[1]).val().split(',');
                        let i = ids.indexOf(arr[0]);
                        ~i && (ids[i] = res.id);
                        parent.$('#' + arr[1]).val(ids.join(','));
                        parent.$(".cropper[data-input-id=" + arr[0] + "]").attr('data-input-id', res.id);
                    } else {
                        console.log(res);
                    }
                },
